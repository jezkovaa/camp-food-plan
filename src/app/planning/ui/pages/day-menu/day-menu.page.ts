...
import { IonContent, IonHeader, IonTitle, IonToolbar, IonButtons, IonBackButton, IonButton, IonIcon, IonPopover, IonDatetime, IonDatetimeButton, IonModal } from '@ionic/angular/standalone';
...
import { IDayMeal, IDayMenu } from 'src/app/planning/data/interfaces/day-menu.interface';
...
@Component({
  ...
  standalone: true,
  imports: [IonModal, IonDatetimeButton, IonDatetime, IonPopover,
    IonIcon,
    IonButton,
    IonBackButton,
    IonButtons,
...
export class DayMenuPage implements OnInit {
  ...
  ngOnInit() {
    ...
    this.route.paramMap.pipe(
      switchMap(params => {
        ...
        return forkJoin({
          dayMenu: this.eventService.getEventMenuById(this.eventId, dayMenuId),
  ...
  addMeal(course: Course) {
    ...
    this.router.navigate(['tabs/planning/events/', this.eventId, 'menu', 'add-meal', course]);
  }
  ...
  private redirectToMenu(date: Date) {
    ...
    this.eventService.getEventMenuForDate(this.eventId, date).subscribe({
